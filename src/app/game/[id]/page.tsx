...
async function fetchNextMetObjectId() {
  const cache: GameCache =
    globalThis.__gameCache || (globalThis.__gameCache = {});
  ...
  if (!response.ok) {
    console.error(
      `Failed to fetch data for ID ${randomId}, status: ${response.status}`
    );
  ...
  if (!data.objectID || !data.gallery) {
    console.log(
      `ID ${randomId} is not a valid Met object or does not have a gallery`
    );
...
async function fetchGameData(id: number) {
  ...
  const response = await fetch(
    `https://collectionapi.metmuseum.org/public/collection/v1/objects/${mappedObjectId}`
  );
  if (!response.ok) {
